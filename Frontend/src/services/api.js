...
export const getRoleNotifications = async ({ userId, role } = {}) => {
  if (!userId) return [];
  // All roles use the same endpoint — the backend filters by targetUser.id
  const response = await api.get(`/notifications?userId=${userId}`);
  return response.data;
};
...
export const closeTicketN3 = async (interventionId, reportDto) => {
  const response = await api.post(`/technicien/interventions/${interventionId}/close-n3`, reportDto);
  return response.data;
};
...
export const replaceEquipment = async (ticketId, newEquipmentId, technicienId) => {
  // N3 workflow uses the ticket-based endpoint, not the intervention-based one.
  const response = await api.put(`/tickets/${ticketId}/replace-equipment`, {
    newEquipmentId: String(newEquipmentId),
    technicienId: String(technicienId),
  });
  return response.data;
};
