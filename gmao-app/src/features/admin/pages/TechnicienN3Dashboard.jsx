...
export default function TechnicienN3Dashboard() {
  ...
  const stats = {
    ...
    critical: availableTickets.filter(t => String(t.priorite).toUpperCase() === 'HIGH').length +
      activeInterventions.filter(i => String(i.ticket?.priorite).toUpperCase() === 'HIGH').length,
    equipmentsTotal: equipments.length,
    equipmentsAvailable: equipments.filter(e => e.disponible === true).length
  };
  ...
  const getPriorityBadge = (priorite) => {
    ...
    if (p === 'HIGH' || p === 'HAUTE') return <span className="n3-badge n3-badge-high">Critique</span>;
      ...
      <aside className="n3-sidebar">
        <div className="n3-sidebar-header">
          <div className="n3-logo-row">
            <div className="n3-logo-icon">OCP</div>
            <div>
              <h1 className="n3-logo-title">HELP-DESK GMAO</h1>
              <p className="n3-logo-sub">Technicien N3</p>
            </div>
          </div>
        </div>

        <nav className="n3-nav">
          <span className="n3-nav-section-label">Centre d'expertise</span>
          <button
          ...
          <button
            className={`n3-nav-btn ${activeTab === 'notifications' ? 'active' : ''}`}
            onClick={() => setActiveTab('notifications')}
          >
            <Bell size={18} />
            Notifications
            {unreadCount > 0 && (
              <span className="n3-nav-badge">{unreadCount}</span>
            )}
          </button>
        ...
        <div className="n3-sidebar-footer">
          <div className="n3-user-card">
            <div className="n3-user-avatar">{userInitials}</div>
            <div>
        ...
        <header className="n3-header">
          <div className="n3-header-left">
            <div className="n3-header-breadcrumb">
              GMAO OCP <span>/</span> Service Informatique N3
            </div>
            <h2 className="n3-header-title">
              {activeTab === 'dashboard' && 'Centre de Contrôle & Supervision N3'}
              {activeTab === 'interventions' && 'Gestion des Expertises Physiques'}
              {activeTab === 'profile' && 'Informations Profil IT'}
              {activeTab === 'notifications' && 'Historique des Alertes N3'}
            </h2>
          </div>

          <div className="n3-header-actions">
            {/* Click bell to open notifications tab */}
            <button
              className="n3-icon-btn relative"
              onClick={() => setActiveTab('notifications')}
              title="Centre d'alertes"
            >
              <Bell size={18} />
              {unreadCount > 0 && (
                <span className="absolute top-1 right-1 h-4 w-4 bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center animate-pulse">
                  {unreadCount}
                </span>
              )}
            </button>
            <button
              className="n3-icon-btn"
              ...
              <div className="n3-fade-in">
                  ...
                  <div className="n3-workflow-banner active">
                    ...
                    <div className="n3-workflow-banner-actions">
                      ...
                      {isEquipmentReplaced() && (
                        ...
                        >
                          <FileText size={16} /> Clôturer l'Intervention
                        </button>
                      )}
                    </div>
                  </div>
                ...
                <div className="n3-stats-grid">
                  ...
                  >
                    ...
                    <div className="n3-stat-icon"><Inbox size={24} /></div>
                  </div>
                  ...
                  <div className="n3-stat-card red">
                    ...
                    <div className="n3-stat-icon"><AlertTriangle size={24} /></div>
                  </div>

                  <div
                    className="n3-stat-card indigo"
                    onClick={() => { setActiveTab('interventions'); setActiveSubFilter('EN_COURS'); }}
                  >
                    <div>
                    ...
                    <div className="n3-stat-icon"><Truck size={24} /></div>
                  </div>
                  ...
                  >
                    ...
                    <div className="n3-stat-icon"><CheckCircle size={24} /></div>
                  </div>
                  ...
                  <div className="n3-stat-card amber">
                    ...
                    <div className="n3-stat-icon"><Settings size={24} /></div>
                  </div>
                </div>
                ...
                <div className="n3-card">
                  ...
                  {availableTickets.length === 0 ? (
                    <div className="n3-empty">
                      <Inbox size={36} className="n3-empty-icon" />
              ...
              <div className="n3-fade-in">
                ...
                <div className="n3-filters-row">
                  <div style={{ display: 'flex', gap: '0.4rem', background: '#e2e8f0', padding: '0.25rem', borderRadius: '8px' }}>
                    {[
                      ...
                      { key: 'PENDING', label: 'À prendre (N3)', dot: stats.available > 0 },
                      { key: 'EN_COURS', label: 'En cours', count: stats.active > 0 ? stats.active : null },
                      { key: 'HISTORIQUE', label: 'Mon Historique' }
                    ].map(item => (
                      <button
                        key={item.key}
                        className={`n3-tab-sub-btn ${activeSubFilter === item.key ? 'active' : ''}`}
                        ...
                        style={{
                          ...
                          borderRadius: '6px',
                          fontWeight: 700,
                          backgroundColor: activeSubFilter === item.key ? '#2563eb' : 'transparent',
                          color: activeSubFilter === item.key ? '#fff' : '#475569'
                        }}
                      >
                        {item.label}
                        {item.dot && <span style={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: '#ef4444', marginLeft: 4, display: 'inline-block' }} />}
                        {item.count != null && <span style={{ marginLeft: 6, background: 'rgba(255,255,255,0.2)', padding: '1px 5px', borderRadius: '4px', fontSize: '0.7rem' }}>{item.count}</span>}
                      </button>
                    ))}
                  </div>

                  <div className="n3-search-box" style={{ marginLeft: 'auto' }}>
                ...
                {activeSubFilter === 'ALL' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    ...
                    {activeInterventions.length > 0 ? (
                      <div className="n3-card" style={{ borderLeft: '4px solid #2563eb' }}>
                        <div className="n3-card-header">
                          <div>
                            <h3 className="n3-card-title" style={{ color: '#2563eb' }}>⚡ Intervention N3 active — Diagnostic en cours</h3>
  ...
  // Helper to update report forms
  function setFormDetails(field, value) {
    setReportForm(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  if (tickets.length === 0) {
    return (
      <div className="n3-empty" style={{ padding: '2rem' }}>
        <Inbox size={32} className="n3-empty-icon" />
        <p className="n3-empty-title">Aucun ticket en attente</p>
                    ...
                    <td colSpan="7" style={{ background: '#f8fafc', padding: '1rem' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', background: '#fff', border: '1px solid #cbd5e1', borderRadius: '10px', padding: '1rem' }}>
                        <div>
                          ...
                          <p style={{ margin: 0, fontSize: '0.8rem', background: '#f1f5f9', padding: '0.5rem', borderRadius: '6px', whiteSpace: 'pre-wrap' }}>
                            {reports[ticket.id]?.n1 || 'Chargement du rapport N1...'}
                          </p>
                        </div>
                        ...
                        <div>
                          ...
                          <p style={{ margin: 0, fontSize: '0.8rem', background: '#fef3c7', padding: '0.5rem', borderRadius: '6px', whiteSpace: 'pre-wrap' }}>
                            {reports[ticket.id]?.n2 || 'Chargement du rapport N2...'}
                          </p>
                        </div>
            ...
            <div className="n3-checklist-step n3-fade-in">
              ...
              <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                <button className="n3-btn n3-btn-secondary" onClick={onSave}>
                  <Database size={15} /> Sauvegarder BDD
                </button>
